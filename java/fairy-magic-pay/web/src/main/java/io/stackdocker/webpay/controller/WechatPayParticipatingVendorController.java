package io.stackdocker.webpay.controller;
...
import io.stackdocker.webpay.help.ZxingClientQrcodeException;
 ...
 * @author Javen
 * 商户模式下微信支付
 * https://github.com/Javen205/IJPay-Demo/blob/master/src/main/java/com/ijpay/controller/weixin/WxPayController.java
 * Server side of business unit mode
 */
@Controller
public class WechatPayParticipatingVendorController {
...
//		return apiConfig;
        return wxpayClient.getApiConfig();
	}
    
    private String msg = null;
    
    @ModelAttribute("showMsg")
    String showMsg() {
        return msg;
    }

    @ModelAttribute("parentPath")
    String parentPath() {
        return "/wxpay";
    }
...
//		renderText("欢迎使用IJPay 商户模式下微信支付  - by Javen");
        ...
        logger.debug("feed default pre-payment:" + prePayment);
        return "booking/form";
	}
    ...
    public String prepay(@Valid final PrePayment prePayment, final BindingResult bindingResult, final ModelMap model, 
        ...
        if (bindingResult.hasErrors()) {
            logger.warn("Fail to pre-pay as binding errors");
            msg = "Fail to pre-pay, try again";
            return "redirect:/wxpay";
        }
        
        String arg = "go-to-docker: webpay";
        String item = "mooc-course";
        String ip = request.getRemoteAddr(); //String ipaddresses = request.getHeader("X-FORWARDED-FOR");
        Map<String, String> resp;
        try {
            resp = wxpayClient.prePay(prePayment.getBilling(), prePayment.getSummary().intValue(),
                    arg, item, ip);
        } catch (WeChatUnifiedOrderApiException e) {
        ...
        String qrcode = resp.get("code_url");
        logger.info("QRCODE:" + qrcode);
        msg = null;        
        model.clear();
        
        return "redirect:/wechatpay/anh";
    }
    
    @RequestMapping("/wxpay/qrcode")
    public ResponseEntity<byte[]> qrcode(@RequestParam(value="billing", required=true) UUID billing) {
    	if (!cachedPayments.containsKey(billing)) {
			HttpHeaders headers = new HttpHeaders();
			headers.set("Content-Type", "image/png");
    		return new ResponseEntity<>(null, headers, HttpStatus.OK);
		}
		String qrcodecontent = cachedPayments.get(billing).getQrcode();
        BufferedImage bi;

        try {
			bi = qrcodeClient.encode(qrcodecontent, BarcodeFormat.QR_CODE, 3, ErrorCorrectionLevel.H, "png", 200, 200,
					PathKit.getWebRootPath() + File.separator + billing.toString());
		} catch (ZxingClientQrcodeException e) {
			HttpHeaders headers = new HttpHeaders();
			headers.set("Content-Type", "image/png");
			return new ResponseEntity<>(null, headers, HttpStatus.OK);
		}
...
//        InputStream is = new FileInputStream(filename);
    ...
    HttpSession getSession() {return null;}
    String getPara(String name) {return "";}
    void renderJson(AjaxResult json){}
	HttpServletRequest getRequest(){return null;}
	void redirect(String url){}
	void renderJson(String json){}
	String getSessionAttr(String name) {return "";}
	Enumeration<String> getParaNames() {return null;}
